...
- 선호도 5종:
  ...
  | 블루오션 | 20% | 45% | 10% | 25% |
  | 균형(상관없음) | 40% | 30% | 15% | 15% |
- `gu` 타입 결과: rawResults에 가중치 적용 후 재정렬 (프론트 클라이언트사이드, 백엔드 변경 없음)
...
- **탭 스트립**: 3버튼 pill UI, 선택된 탭 흰 배경 + 파란 텍스트 + 그림자

| 탭 | 포함 카드 |
|----|-----------|
| 기본 분석 | 종합 AI 점수 게이지, 핵심지표 2×2, 등급 분포, 예산, 소분류 안내 |
| 수익 예측 | 생존율 추정, 재무 추정 (권리금·ROI·손익분기) |
| 트렌드 | 배달·오프라인 비중, 네이버 검색 트렌드 |

- **gu 타입**: 3탭 구조 완전 구현
- **dong 타입**: 3탭 구조 미완 (중단됨, 현재 기존 스택 레이아웃 유지)
