"""
크롤링 데이터(naver_place_info.csv) → 행정동×업종 피처 집계
→ final_dataset.csv에 조인하여 저장
"""
import numpy as np
import pandas as pd
...
DATA_PATH  = ROOT / "data" / "processed_data" / "final_dataset.csv"
OUT_PATH   = ROOT / "data" / "processed_data" / "final_dataset.csv"
...
print("=" * 50)
print("3. 행정동×업종 집계")
agg = crawl.groupby(["행정동명", "통합카테고리"]).agg(
...
# 샘플 수가 너무 적으면(3개 미만) 업종 평균으로 대체
cat_agg = crawl.groupby("통합카테고리").agg(
...
print(f"  집계 결과: {len(agg):,}개 (행정동×업종)")
print(f"  커버 행정동: {agg['행정동명'].nunique()}")

print("=" * 50)
print("4. final_dataset에 조인")
df = pd.read_csv(DATA_PATH, encoding="utf-8-sig")
print(f"  기존 final_dataset: {len(df):,}행, {len(df.columns)}컬럼")
...
df = df.merge(agg, on=["행정동명", "통합카테고리"], how="left")

# 조인 후 null → 업종 전체 평균으로 보완
for col in ["crawl_avg_price", "crawl_price_range", "crawl_1인비율",
            "crawl_회식비율", "crawl_목적다양성", "crawl_menu_count"]:
