...
# 업종별 보완 업종 (시너지) — 현재 DB 카테고리 기준
COMPLEMENT_MAP = {
    "한식":            ["카페", "주점", "편의점", "분식/간식", "슈퍼마켓"],
    "분식/간식":       ["카페", "한식", "편의점", "베이커리/디저트"],
    "베이커리/디저트": ["카페", "분식/간식", "한식"],
    "중식":            ["카페", "주점", "편의점"],
    "일식":            ["카페", "주점", "편의점"],
    "양식/기타외식":   ["카페", "주점", "편의점"],
    "치킨전문점":      ["주점", "편의점", "한식"],
    "패스트푸드":      ["카페", "편의점", "분식/간식"],
    "카페":            ["한식", "분식/간식", "화장품", "일반의류"],
    "주점":            ["한식", "분식/간식", "일식", "편의점"],
    "편의점":          ["한식", "카페", "분식/간식", "주점"],
    "슈퍼마켓":        ["한식", "편의점"],
    "일반의류":        ["화장품", "미용실", "카페"],
    "화장품":          ["일반의류", "미용실", "카페", "피부관리실"],
    "피부관리실":      ["화장품", "일반의류", "미용실"],
    "미용실":          ["화장품", "일반의류", "카페"],
    "일반의원":        ["의약품", "편의점"],
    "의약품":          ["일반의원", "편의점"],
...
# 소분류 기반 카테고리 보정 (build_merged_dataset.ipynb와 동일한 로직)
def apply_소분류_override(mid, 소분류):
    if mid == "기타 간이":
        if 소분류 == "치킨":
            return "치킨전문점"
        if 소분류 in {"피자", "버거", "토스트/샌드위치/샐러드"}:
            return "패스트푸드"
        if 소분류 in {"빵/도넛", "떡/한과", "아이스크림/빙수"}:
            return "베이커리/디저트"
    elif mid == "의원":
        if 소분류 == "치과의원":
            return "치과의원"
        ...
        if 소분류 == "당구장":
            return "당구장"
        if 소분류 in {"헬스장", "종합 스포츠시설", "기타 스포츠시설 운영업",
                      "수영장", "볼링장", "스쿼시/라켓볼장", "탁구장", "테니스장"}:
            return "스포츠클럽"
        if 소분류 in {"에어로빅/댄스", "무도학원", "기타 스포츠 강습업", "수영 강습업",
                      "골프 강습업", "태권도/무술 강습업", "피트니스 강습업"}:
            return "스포츠 강습"
        ...
        if 소분류 == "가방 소매업":
            return "가방"
        if 소분류 in {"실/섬유제품 소매업", "침구류/커튼 소매업"}:
            return "섬유제품"
        if 소분류 in {"기타 의류 소매업", "여성 의류 소매업", "남성 의류 소매업",
                      "유아용 의류 소매업", "액세서리/잡화 소매업", "가발 소매업"}:
            return "일반의류"
    elif mid == "가전·통신 소매":
        if 소분류 == "핸드폰 소매업":
            return "핸드폰"
        if 소분류 == "컴퓨터/소프트웨어 소매업":
            return "컴퓨터및주변장치판매"
        if 소분류 == "가전제품 소매업":
            return "가전제품"
    elif mid == "식료품 소매":
        ...
        if 소분류 in {"채소/과일 소매업"}:
            return "청과상"
        if 소분류 in {"반찬/식료품 소매업"}:
            return "반찬가게"
        if 소분류 in {"수산물 소매업", "건어물/젓갈 소매업"}:
            return "수산물판매"
        if 소분류 in {"곡물/곡분 소매업"}:
            return "미곡판매"
    elif mid == "기타 보건":
        if 소분류 in {"의료기기 소매업", "의료기기 임대업"}:
            return "의료기기"
    elif mid == "의약·화장품 소매":
        if 소분류 == "의료기기 소매업":
            return "의료기기"
...
def load_quarter(quarter_name):
    ...
    df["통합카테고리"] = df["상권업종중분류명"].map(cat_dict).fillna("기타")
    # 소분류 세분화가 필요한 중분류 전체에 오버라이드 적용
    override_mids = {
        "기타 간이", "의원", "이용·미용", "유원지·오락", "스포츠 서비스",
        "섬유·의복·신발 소매", "가전·통신 소매", "식료품 소매",
        "기타 보건", "의약·화장품 소매", "수의", "기타 교육", "전문 디자인",
    }
    override_mask = df["상권업종중분류명"].isin(override_mids)
    if override_mask.any():
        overrides = df.loc[override_mask].apply(
            lambda r: apply_소분류_override(r["상권업종중분류명"], r["상권업종소분류명"]) or r["통합카테고리"],
            axis=1,
        )
        df.loc[override_mask, "통합카테고리"] = overrides
    return df
